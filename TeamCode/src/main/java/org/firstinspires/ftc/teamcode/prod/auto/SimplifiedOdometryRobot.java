...
package org.firstinspires.ftc.teamcode.prod.auto;
...
public class SimplifiedOdometryRobot {
    ...
    private static final double DRIVE_GAIN          = 0.29;    // Strength of axial position control
    private static final double DRIVE_ACCEL         = 2.0;     // Acceleration limit.  Percent Power change per second.  1.0 = 0-100% power in 1 sec.
    private static final double DRIVE_TOLERANCE     = 3.5;     // Controller is is "inPosition" if position error is < +/- this amount
    private static final double DRIVE_DEADBAND      = 2.0;     // Error less than this causes zero output.  Must be smaller than DRIVE_TOLERANCE
    private static final double DRIVE_MAX_AUTO      = 0.6;     // "default" Maximum Axial power limit during autonomous

    private static final double STRAFE_GAIN         = 0.21;    // Strength of lateral position control
    ...
    private static final double YAW_GAIN            = 0.18;    // Strength of Yaw position control
    ...
    // my subsystems, i don't care, fuck everything and everyone
    public LiftPID liftPID = new LiftPID();
    public ArmPID armPID = new ArmPID();
     ...
     */
    public void initialize(boolean showTelemetry, HardwareMap hardwareMap)
    {
        ...
        leftFrontDrive  = setupDriveMotor("lf", DcMotor.Direction.REVERSE);
        rightFrontDrive = setupDriveMotor("rf", DcMotor.Direction.FORWARD);
        leftBackDrive  = setupDriveMotor( "lr", DcMotor.Direction.REVERSE);
        rightBackDrive = setupDriveMotor( "rr",DcMotor.Direction.FORWARD);
        ...
        driveEncoder = myOpMode.hardwareMap.get(DcMotor.class, "lf");
        strafeEncoder = myOpMode.hardwareMap.get(DcMotor.class, "rr");
...
//        aMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);  // Requires motor encoder cables to be hooked up.
    ...
    public void driveEncoder(double distance, double power, long holdTime) {
        distance = Math.abs(distance);

        while (myOpMode.opModeIsActive() && readSensors()){

            // implement desired axis powers
            moveRobotEncoder(distance < 0 ? -power : power, 0);

            int currentPos = Math.abs(driveEncoder.getCurrentPosition());

            // Time to exit?
            if (currentPos > distance - 2 && distance + 2 < currentPos) {
                break;
            }
        }
        stopRobotEncoder();
        myOpMode.sleep(holdTime);
    }
    ...
    public void strafeEncoder(double distance, double power, long holdTime) {
        distance = Math.abs(distance);

        while (myOpMode.opModeIsActive() && readSensors()){

            // implement desired axis powers
            moveRobotEncoder(0, distance < 0 ? -power : power);

            int currentPos = Math.abs(strafeEncoder.getCurrentPosition());
    ...
    public void moveRobotEncoder(double drive, double strafe){
    ...
    public void customMoveRobotEncoder(double lf, double rf, double lr, double rr){
        ...
        leftFrontDrive.setPower(lf);
        rightFrontDrive.setPower(rf);
        leftBackDrive.setPower(lr);
        rightBackDrive.setPower(rr);
    }

    public void turnRobotEncoder(double distance, double power, long holdTime){
        distance = Math.abs(distance);

        while (myOpMode.opModeIsActive() && readSensors()){

            // implement desired axis powers
            customMoveRobotEncoder(power, -power, power, -power);
    ...
    public void stopRobotEncoder() {
        moveRobotEncoder(0, 0);
    }
